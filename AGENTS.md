...
- 用户当前要求的目标、交付物和明确排除项是实施边界。任务规模大、提交多或需要较长时间本身不是停止理由；只要仍在批准范围内，主 Agent 应持续组织执行和验证。
- Superpowers、子 Agent、设计、计划、TDD 和代码审查是完成任务的方法，不构成新增功能、扩大威胁模型、进入下一里程碑或追加 GitHub 发布操作的授权。
- 适合并行的独立工作应使用子 Agent 提速，数量由任务结构决定。主 Agent 必须为每个子 Agent 指定清晰且不重叠的范围，并负责依赖协调、结果复核和最终交付；存在共享文件、共享状态或前置依赖时必须串行或明确协调。
- 新发现按与当前目标的关系处理：阻断交付或破坏当前正确性的问题在原范围内修复；非阻塞增强、极端边界、相邻系统问题和长期加固进入 backlog 或最终风险说明，不得自动实施。
- 审查只判断当前交付是否符合已批准要求。不得形成“审查发现范围外问题 → 扩大设计和实现 → 再审查 → 继续扩大”的递归循环，也不得用新的安全、许可、架构或商业化议题替换用户原本要求的交付。
- 已有且仍适用的批准设计或计划时直接执行，不重复设计、规划或要求用户再次确认同一决定；只有发现真实矛盾、缺失前提或范围变化时才回到决策阶段。
- 达到当前交付物并完成风险相称的验证后立即收口。`commit`、`push`、PR、merge、tag、Release 和部署分别授权；完成其中一步不得自行升级到后续步骤，也不得把单一步骤扩张为完整发布工程。
- “继续”只授权此前已经明确说明的当前范围。阶段更新保持简短，只报告结果、真实阻塞和需要项目所有者决定的事项。
...
一个主 Agent 负责读取规则和现有计划、调用适用的 Superpowers skill，并在用户已批准的范围内自动组织执行、审查、修复和验证。独立且不修改相同区域的任务可以并行；存在依赖或文件重叠的任务必须串行。用户不需要手动把任务复制给多个子 Agent，子 Agent 也不得把分配到的子任务扩展为新的项目目标。
