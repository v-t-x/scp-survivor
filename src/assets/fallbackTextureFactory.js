...
function rigPoint(frame, forward, side) {
  const direction = RIG_DIRECTIONS[frame];
  const centerX = frame * 96 + 48;
  const centerY = 48;
  return {
    x: Math.round(centerX + direction.x * forward - direction.y * side),
    y: Math.round(centerY + direction.y * forward + direction.x * side)
  };
}

function fillRigBody(graphics, frame, start, end, halfWidth, color) {
  graphics.fillStyle(color, 1);
  graphics.fillPoints([
    rigPoint(frame, start, -halfWidth),
    rigPoint(frame, end, -halfWidth),
    rigPoint(frame, end, halfWidth),
    rigPoint(frame, start, halfWidth)
  ], true);
}

function drawRigPivot(graphics, frame) {
  const pivot = rigPoint(frame, 0, 0);
  graphics.fillStyle(0x10161d, 1);
  graphics.fillCircle(pivot.x, pivot.y, 12);
  graphics.fillStyle(0x526170, 1);
  graphics.fillCircle(pivot.x, pivot.y, 9);
  graphics.fillStyle(0x202a35, 1);
  graphics.fillCircle(pivot.x, pivot.y, 5);
}

function drawPistolRigFrame(graphics, frame) {
  drawRigPivot(graphics, frame);
  fillRigBody(graphics, frame, -4, 31, 9, 0x202832);
  fillRigBody(graphics, frame, 5, 29, 5, 0x667585);
  graphics.lineStyle(1, 0x111820, 1);
  for (const channel of [-6, -3, 0, 3, 6]) {
    const start = rigPoint(frame, 12, channel);
    const end = rigPoint(frame, 29, channel);
    graphics.lineBetween(start.x, start.y, end.x, end.y);
  }
  for (const [side, color] of [[-11, 0x7a3037], [11, 0x315e6d]]) {
    const housing = rigPoint(frame, 1, side);
    graphics.fillStyle(color, 1);
    graphics.fillCircle(housing.x, housing.y, 2);
  }
}

function drawBreacherRigFrame(graphics, frame) {
  drawRigPivot(graphics, frame);
  fillRigBody(graphics, frame, -5, 31, 12, 0x1c242d);
  fillRigBody(graphics, frame, 7, 25, 15, 0x596979);
  fillRigBody(graphics, frame, 13, 32, 8, 0x252f39);
  graphics.lineStyle(3, 0x91a0ad, 1);
  const sleeveStart = rigPoint(frame, 8, -13);
  const sleeveEnd = rigPoint(frame, 8, 13);
  graphics.lineBetween(sleeveStart.x, sleeveStart.y, sleeveEnd.x, sleeveEnd.y);
  graphics.fillStyle(0x384756, 1);
  graphics.fillPoints([
    rigPoint(frame, -3, 11),
    rigPoint(frame, 12, 12),
    rigPoint(frame, 10, 22),
    rigPoint(frame, -5, 20)
  ], true);
  const status = rigPoint(frame, 0, -14);
  graphics.fillStyle(0x7a3037, 1);
  graphics.fillCircle(status.x, status.y, 2);
}

function drawTeslaRigFrame(graphics, frame) {
  drawRigPivot(graphics, frame);
  fillRigBody(graphics, frame, -5, 27, 13, 0x202832);
  fillRigBody(graphics, frame, 0, 25, 9, 0x526170);
  graphics.lineStyle(4, 0x9b6538, 1);
  for (const side of [-7, 7]) {
    const coilStart = rigPoint(frame, 3, side);
    const coilEnd = rigPoint(frame, 29, side);
    graphics.lineBetween(coilStart.x, coilStart.y, coilEnd.x, coilEnd.y);
  }
  graphics.fillStyle(0x315e6d, 1);
  for (const forward of [5, 11, 17, 23]) {
    for (const side of [-11, 11]) {
      const node = rigPoint(frame, forward, side);
      graphics.fillCircle(node.x, node.y, 2);
    }
  }
}
...
export function generateFallbackTextures(scene) {
  ...
  ensureTexture(scene, TEXTURES.weaponRigPistol, () => {
    graphics.clear();
    for (let frame = 0; frame < 8; frame += 1) drawPistolRigFrame(graphics, frame);
    graphics.generateTexture(TEXTURES.weaponRigPistol, 768, 96);
    const texture = scene.textures.get(TEXTURES.weaponRigPistol);
    texture.add(0, 0, 0, 0, 96, 96);
    texture.add(1, 0, 96, 0, 96, 96);
    texture.add(2, 0, 192, 0, 96, 96);
    texture.add(3, 0, 288, 0, 96, 96);
    texture.add(4, 0, 384, 0, 96, 96);
    texture.add(5, 0, 480, 0, 96, 96);
    texture.add(6, 0, 576, 0, 96, 96);
    texture.add(7, 0, 672, 0, 96, 96);
  });

  ensureTexture(scene, TEXTURES.weaponRigBreacher, () => {
    graphics.clear();
    for (let frame = 0; frame < 8; frame += 1) drawBreacherRigFrame(graphics, frame);
    graphics.generateTexture(TEXTURES.weaponRigBreacher, 768, 96);
    const texture = scene.textures.get(TEXTURES.weaponRigBreacher);
    texture.add(0, 0, 0, 0, 96, 96);
    texture.add(1, 0, 96, 0, 96, 96);
    texture.add(2, 0, 192, 0, 96, 96);
    texture.add(3, 0, 288, 0, 96, 96);
  ...
  ensureTexture(scene, TEXTURES.weaponRigTesla, () => {
    graphics.clear();
    for (let frame = 0; frame < 8; frame += 1) drawTeslaRigFrame(graphics, frame);
    graphics.generateTexture(TEXTURES.weaponRigTesla, 768, 96);
    const texture = scene.textures.get(TEXTURES.weaponRigTesla);
    texture.add(0, 0, 0, 0, 96, 96);
    texture.add(1, 0, 96, 0, 96, 96);
    texture.add(2, 0, 192, 0, 96, 96);
    texture.add(3, 0, 288, 0, 96, 96);
