...
- 与 SCP Foundation、SCP-049 及其他 SCP 衍生创意表达有关的内容，在最终项目许可复核通过的前提下按 [CC BY-SA 3.0 全文](../LICENSES/CC-BY-SA-3.0.txt) 提供；
...
登记记录表明这些项目生成素材未使用第三方图片输入，并保留了提示、处理、尺寸、准入和来源标识。OpenAI 输出可能不唯一；当前记录不能证明素材唯一、不侵权或已取得商业发布许可，商业发布前仍须人工复核服务条款、来源和成品。

### SCP 衍生内容

SCP 世界观、SCP-049 及其他可识别的 SCP 衍生表达与独立软件代码分开处理，候选范围为 CC BY-SA 3.0，并必须满足具体条目作者署名和相同方式共享要求。首次公开分发前应按实际使用的条目、文字、图像、名称和表达逐项复核。
