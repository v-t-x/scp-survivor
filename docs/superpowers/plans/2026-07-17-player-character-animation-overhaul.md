...
**Architecture:** `src/art/characterPresentation.js` 继续作为唯一角色表现入口，新增按角色 ID 注册的只读配置、纯函数动作分类器和原子动画注册；`src/art/openingVisualContract.js` 同时保留 legacy/new 命名合同，并只在 Gate 3 接受时切换权威默认；`src/scene/world.js` 只消费解析出的纹理与显示缩放，仍先建立 24×24 Arcade Body，再走现有保体缩放路径。素材按“三剪影 → 单方向真实游戏原型 → 四方向完整表”推进，Gate 2 先归一化 legacy 兼容行，旧 `player-opening-sheet` 是一级动画 fallback，`player.png` 是二级静态 fallback；浏览器状态通过 query-gated、可恢复且不接触 RNG/存档的开发态驱动复现。
...
- 新角色显示缩放为 `1.0`；Gate 2 嵌入 64×64 prototype 的 legacy 兼容帧先用 nearest-neighbor 归一化到可见高度 44–50 再按 `1.0` 显示；只有运行时回退到原始 48×48 legacy sheet 或旧静态图时保持 `1.2`；所有路径都必须保持玩家 Arcade Body 为 24×24。
...
`src/main.js` 也是高冲突共享文件。本阶段唯一允许的变更是一个 `import.meta.env.DEV && URLSearchParams(...).get("playerCharacterSmoke") === "1"` 动态 import；默认开发 URL 和生产 build 都不安装、不暴露 smoke bridge。影响范围限于显式 smoke URL；验证方法是源码门禁、driver 单元测试、普通 URL 上全局入口不存在、smoke URL 上入口存在、build 后产物不出现 `__SCP_PLAYER_CHARACTER_SMOKE__`。不得顺带调整 Scene 初始化、update、restart 或 mixin 顺序。
...
| `src/art/playerCharacterVisualStateDriver.js` | 新建 | 提供 query-gated、可恢复、零 RNG/存档访问的 24 状态浏览器表现驱动 |
| `src/scene/world.js` | 修改 | 按默认角色 ID 解析纹理/缩放，继续创建相同 24×24 body |
| `src/main.js` | 修改 | 仅在 `import.meta.env.DEV` 且 URL 含 `playerCharacterSmoke=1` 时动态安装表现驱动 |
...
| `test/player-character-visual-state-driver.test.js` | 新建 | 锁定 24 个状态、暂停/恢复、body 不变、异常 finally 恢复以及不访问 RNG/localStorage |
...
- `test_prototype_normalizes_legacy_rows_to_the_new_visible_height`：fixture 的 legacy 帧可见高度固定为 40–42；断言 down 行来自新 board，legacy down hit 和 left/right/up 经 shared nearest-neighbor 放大后每帧为 44–50、baseline y=56，四行可见高度中位数最大差不超过 2 像素，且不再逐像素等于未归一化输入。
...
- 当前 `player-opening-sheet.png` 的 48 个 alpha bbox 已验证为可见高度 40–42；运行时原表乘 `1.2` 后为 48–50.4。工具测试必须保留这组真实输入断言，防止把未放大的 legacy 像素直接塞进 1.0 prototype。
...
- `prototype` 先按 legacy 原生行序 `down,left,right,up` 分别计算 12 帧 union bbox；每行使用唯一 shared scale `48 / union_bbox_height`，以 `Image.Resampling.NEAREST` 缩放同一 union crop，再水平居中并把脚底放到 y=56。禁止直接把 48×48 像素复制到 64×64 cell，也禁止逐帧单独求 scale。
- `prototype` 的 down 列 0–27 来自新板；down hit 28–29 来自已归一化 legacy down hit；其他三行来自各自已归一化的 legacy 原生方向。已归一化的旧 `move 4–9` 依次复制到四个临时 locomotion block，仅为 Gate 2 运行兼容，不计入新动作验收；right 必须取 legacy 第三行，不得再次镜像 left。
...
Expected: FAIL，缺少新导出、legacy/new 权威合同、新 texture key、120 帧 layout 和 fallback 事务行为。
...
- Produces: down 方向 idle/forward/backward/strafeLeft/strafeRight 的真实游戏原型；归一化 legacy 兼容方向；`window.__SCP_PLAYER_CHARACTER_SMOKE__` 的显式开发态入口

- [ ] **Step 1: 先写可恢复浏览器状态驱动失败测试**

在 `test/player-character-visual-state-driver.test.js` 用纯 stub 锁定：`PLAYER_CHARACTER_VISUAL_STATES` 精确包含 `down,left,right,up × idle,forward,backward,strafeLeft,strafeRight,hit` 共 24 项；每项 angle/velocity 进入 `getPlayerMotion()` 后得到同名 locomotion；玩家仍处于 hit tint 时创建 driver 明确失败；`applyState()` 只改表现输入并返回 animation/texture/scale/body 快照；连续切换四方向后 body 的 x/y/width/height/offset 不变；`restore()` 幂等并恢复 angle、velocity、hit deadline、facing、flip、animation progress、`scene.isPaused` 和 physics pause 状态。

把 `Math.random` 临时替换为抛错函数，并给 `globalThis.localStorage` getter 一个抛错 stub；24 个状态仍必须全绿，证明驱动完全不读取 RNG 或存档。注入会抛错的 `syncPresentation`，断言 `applyState()` 在 catch 内先恢复再原样抛错。源码再锁定：
...
export function createPlayerCharacterVisualStateDriver({
  ...
  if (!scene?.player?.body || !scene.player.anims) {
    throw new Error("player character smoke requires an active player sprite");
  }
  const player = scene.player;
  if (player.isTinted) {
    throw new Error("wait for player hit tint to clear before starting character smoke");
  }
  const world = scene.physics?.world;
  const original = {
    scenePaused: scene.isPaused,
    physicsPaused: world?.isPaused === true,
    ...
    presentationHitUntilMs: player.presentationHitUntilMs,
    flipX: player.flipX,
    animationKey: player.anims.currentAnim?.key ?? null,
    animationProgress: player.anims.getProgress?.() ?? 0,
    body: bodySnapshot(player.body)
  };
  let restored = false;
  scene.isPaused = true;
  if (!original.physicsPaused) world?.pause?.();

  function restore() {
    if (restored) return;
    player.body.setVelocity(original.velocityX, original.velocityY);
    scene.elapsedSurvivalMs = original.elapsedSurvivalMs;
    scene.playerFacingAngle = original.playerFacingAngle;
    player.presentationFacing = original.presentationFacing;
    player.presentationHitUntilMs = original.presentationHitUntilMs;
    ...
    if (!original.physicsPaused) world?.resume?.();
    scene.isPaused = original.scenePaused;
    restored = true;
  }

  function applyState(name) {
    if (restored) throw new Error("player character smoke driver already restored");
    const state = PLAYER_CHARACTER_VISUAL_STATES[name];
    if (!state) throw new Error(`unknown player character visual state: ${name}`);
    try {
      scene.elapsedSurvivalMs = 1000;
      scene.playerFacingAngle = state.angle;
      player.body.setVelocity(state.velocity.x, state.velocity.y);
      player.presentationHitUntilMs = state.motion === "hit" ? 1120 : 0;
      syncPresentation(scene);
      const body = bodySnapshot(player.body);
      if (JSON.stringify(body) !== JSON.stringify(original.body)) {
...
export function installPlayerCharacterVisualStateBridge(scene, windowRef = window) {
  ...
  windowRef.__SCP_PLAYER_CHARACTER_SMOKE__?.restore?.();
  Object.defineProperty(windowRef, "__SCP_PLAYER_CHARACTER_SMOKE__", {
    configurable: true,
    value: driver
  });
  const cleanup = () => {
    driver.restore();
    delete windowRef.__SCP_PLAYER_CHARACTER_SMOKE__;
  };
...
if (
  import.meta.env.DEV
  && new URLSearchParams(window.location.search).get("playerCharacterSmoke") === "1"
) {
  import("./art/playerCharacterVisualStateDriver.js")
    ...
    .catch((error) => console.error("[player-character-smoke]", error));
}
```

- [ ] **Step 4: 运行 driver GREEN 并单独提交 smoke seam**

```powershell
& 'C:\Users\24037\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --test test/player-character-visual-state-driver.test.js
npm run build
if (rg -n "__SCP_PLAYER_CHARACTER_SMOKE__" dist) { throw 'smoke bridge leaked into production build' }
git diff --check
git add -- src/art/playerCharacterVisualStateDriver.js src/main.js test/player-character-visual-state-driver.test.js
git diff --cached --name-only
git commit -m "test(art): add reversible player visual state driver"
```

Expected: driver 测试全绿；build 成功；显式检查不抛错，证明生产 build 没有 smoke 全局入口；staged list 精确为三个文件。
...
PNG 门禁固定为 1920×256、8-bit RGBA、120 帧、binary alpha、≤32 色；down 的五个目标 motion 各自帧数为 4/6/6/6/6、translation-normalized hash 全唯一、每帧高 44–50、baseline y=56±1、左右漂移≤2。四行全部 occupied frames 都必须高 44–50、baseline y=56±1；逐行可见高度中位数最大差≤2，防止切换到 legacy 兼容方向时角色缩小。prototype 暂不要求其他三行的新动作差异，也不把 down hit 计入新素材验收。
...
Expected: 1920×256；down 0–27 是新动作；down hit 和其他方向来自按行 shared nearest-neighbor 归一化后的旧表；四行中位可见高度差≤2；没有逐帧缩放或镜像生成。
...
Expected: 1× 1920×256；4× 7680×1024。down 行用于动作质量判断；四行并排用于身份比例、baseline 和方向切换尺寸判断，legacy 行明确标为“归一化兼容内容，不代表最终动作质量”。
...
先在普通 `http://localhost:64126/` 完成标题→武器选择→实战→暂停→restart，并在页面上下文执行 `typeof window.__SCP_PLAYER_CHARACTER_SMOKE__`，Expected: `"undefined"`。再打开 `http://localhost:64126/?playerCharacterSmoke=1`，固定 viewport 960×540；等待入口存在后执行：

```js
const driver = window.__SCP_PLAYER_CHARACTER_SMOKE__;
if (!driver) throw new Error("player character smoke bridge missing");
...
```js
window.__SCP_PLAYER_CHARACTER_SMOKE__?.restore();
```

执行顺序固定为每个方向的 `idle,forward,backward,strafeLeft,strafeRight,hit`，方向顺序 `down,left,right,up`。保存全部 24 份 JSON 到 `.superpowers/sdd/player-character/gate-2/state-snapshots.json`；截图至少包含 down 的五种 locomotion，以及 down→left→right→up 的 idle 和 forward，共 11 张。不得通过自然刷怪、等待自动攻击、改 Scene 源码、改 `Math.random`、写 `localStorage` 或调用胜负/发奖入口制造状态。

Expected: down 五种动作肉眼不同且无月球漫步；四方向连续切换时人物可见高度保持 44–50、比例无明显跳变、baseline 不跳；24 个返回值 texture/scale 正确；body 始终 24×24 且 x/y/offset 不因状态改变；普通 URL 不暴露 bridge；console error 0、新 sheet warning 0；restore 后原 velocity/facing/animation/pause/physics 状态与入口前快照一致。

- [ ] **Step 12: 用户视觉验收门 2**

向用户提供 1× contact、down 五动作截图、四方向切换对比截图和稳定 `http://localhost:64126/`。请用户判断 down 方向的身份、比例、像素密度、前进/后退/左右横移、滑行和月球漫步，并确认切到 legacy left/right/up 时角色不缩小；兼容方向的动作质量不作为 Gate 2 接受条件。未明确接受前不制作其他方向、不提交 prototype；反馈只修当前原型并重复 Steps 7–12。
...
Expected: 全部 Node/Python 测试 0 fail；build 成功且生产 bundle 不含 smoke bridge；diff 无 whitespace error；`DEFAULT_OPENING_PLAYER_ASSET_ID` 仍为 `legacy`；状态只包含尚未提交的 final candidate、manifest/test/register 修改以及既有 `?? .superpowers/`。
...
用 `http://localhost:64126/?playerCharacterSmoke=1` 和 Task 5 已提交的 `window.__SCP_PLAYER_CHARACTER_SMOKE__` 按同一 24 状态顺序驱动表现；浏览器控制器必须用 `try/finally` 调用 `restore()`。截图和 JSON 快照保存到 `.superpowers/sdd/player-character/gate-3/review/`，不暂存。driver 本身不访问 RNG 或 `localStorage`；真实武器、暂停和 restart 流程通过正常 UI/input 单独执行，不得调用会发奖的胜利/失败内部触发器。
...
```powershell
& 'C:\Users\24037\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' --test test/opening-visual-contract.test.js
```

Expected: FAIL，actual default 仍为 `legacy`；legacy 命名合同本身继续通过。然后只把 `src/art/openingVisualContract.js` 的默认 ID 改为：
...
Expected staged list：final PNG、manifest、opening contract、asset/opening tests、asset register，以及 prototype 删除；默认合同切换与 final 素材同一 commit；不含 `.superpowers/` 或其他文件。
...
Expected: 全部测试 0 fail、build 成功、生产 build 不含 smoke bridge、diff check 通过；工作区只剩 `?? .superpowers/`；分支仍为 `feature/ui-art-overhaul`；没有 merge/push。

- [ ] **Step 2: 最终 runtime smoke 不复用旧证据**

先用普通 URL 验证 smoke 全局入口不存在并重新走标题→选武器→四方向动作→受击→暂停→restart；再用 `?playerCharacterSmoke=1` 新建 driver，依次执行 24 状态并在 `finally` 调用 `restore()`，重新读取 console、texture、animation key 和 body snapshot。若服务已停止，按 Task 5 的端口只读检查后重新启动；不得把历史截图当成当前 HEAD 证据。
