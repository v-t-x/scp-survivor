...
- [v1.6.0 — 收容区焕新](./releases/v1.6.0.md)
...
`docs/superpowers/specs/` 和 `docs/superpowers/plans/` 保存已讨论的设计与实施过程。它们用于审计决策，但只有已批准且仍适用的计划才能授权工作；其内容不能覆盖产品愿景、当前设计或 Agent 治理，也不是 Release 运行或许可核对的依赖。`.superpowers/` 是本地审计工作区，不进入发布提交。

## 根目录标准文件

- `README.md` / `README.en.md`：中英文公开项目首页。
- `CHANGELOG.md`：版本更新记录。
- `LICENSE`：代码、SCP 衍生内容与项目素材的分层许可总览。
- `ATTRIBUTION.md`：SCP、SCP-049 与项目素材候选署名。
