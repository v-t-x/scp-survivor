...
## 2026-09-05 U1 按原蓝图返工（当前本地实现）

项目所有者批准“按你说的返工”，继续适用此前新增 U1 专属正式资产及 manifest 接入授权。本节覆盖下方 2026-09-04 两张 chassis 的当前运行路径与 hero 使用范围；下方旧文件、哈希和历史审查记录全部保留，但旧 PASS 不代表本轮用户视觉验收。没有提交或发布。
...
- 四张 perk、atlas PNG/JSON 均不改；两张旧 chassis 仍保留，但不再由 manifest 加载。名称、数值、状态、按钮文案仍由运行时绘制，底盘没有烘焙 live data。
